...
for(bin = 0; bin < bins; bin++) {
 ...
 if(blockSize >= 1024) {
   if(sharedIdx < 512) {sdata2[sharedIdx] += sdata2[sharedIdx + 512];} __syncthreads();
 }
 if(blockSize >= 512) {
   if(sharedIdx < 256) {sdata2[sharedIdx] += sdata2[sharedIdx + 256];} __syncthreads();
 }
 if(blockSize >= 256) {
   if(sharedIdx < 128) {sdata2[sharedIdx] += sdata2[sharedIdx + 128];} __syncthreads();
 }
 if(blockSize >= 128) {
   if(sharedIdx < 64) {sdata2[sharedIdx] += sdata2[sharedIdx + 64];} __syncthreads();
 }
 if(sharedIdx < 32){
   if(blockSize >= 64) sdata2[sharedIdx] += sdata2[sharedIdx + 32];
   if(blockSize >= 32) sdata2[sharedIdx] += sdata2[sharedIdx + 16];
   if(blockSize >= 16) sdata2[sharedIdx] += sdata2[sharedIdx + 8];
   if(blockSize >= 8) sdata2[sharedIdx] += sdata2[sharedIdx + 4];
   if(blockSize >= 4) sdata2[sharedIdx] += sdata2[sharedIdx + 2];
   if(blockSize >= 2) sdata2[sharedIdx] += sdata2[sharedIdx + 1];
 }
	float sum;
	if (sharedIdx == 0) {
